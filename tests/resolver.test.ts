...
describe('ToolResolver', () => {
  ...
  it('does not artificially boost companion tools by default when companionBoost is 0', () => {
    const catalog = new ToolCatalog();
    const tools: ToolDefinition[] = [
      { name: 'jira_get_issue', description: 'Get Jira issue details', domain: 'jira' },
      { name: 'jira_update_issue', description: 'Update Jira issue fields', domain: 'jira' },
    ...
    catalog.setEmbeddings({
      jira_get_issue: [1.0, 0.0],
      jira_update_issue: [0.0, 0.0], // Unrelated embedding
      slack_send_message: [0.0, 1.0],
    });
    ...
    // Resolve without explicit companionBoost (uses default companionBoost: 0.0)
    const queryVec = new Float32Array([1.0, 0.0]);
    const result = resolver.resolve('fetch issue JIRA-123', queryVec, undefined, {
      alpha: 1.0,
      topK: 3,
    ...
    // Companion bonus must be 0.0
    expect(result.scores['jira_update_issue']).toBe(0);
    expect(result.selectedNames).not.toContain('jira_update_issue');
  });
