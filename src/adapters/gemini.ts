...
export function geminiDeclToImpulseTool(decl: GeminiFunctionDeclaration): ToolDefinition {
  return {
    name: decl.name,
  ...
  return {
    ...
    async filterDeclarations(
      ...
      if (catalog.getToolNames().length === 0) {
        engine.registerToolsSync(declarations.map(geminiDeclToImpulseTool));
      }
