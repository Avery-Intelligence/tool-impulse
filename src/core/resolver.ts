...
export class ToolResolver {
   ...
   * By default, driftThreshold is 0.0, preserving conversational context even when
   * follow-up commands introduce orthogonal vocabulary (e.g. "now email that report to Alice"
   * following "calculate monthly churn in Postgres").
   * If a positive driftThreshold is explicitly configured, prior context is dropped when
  ...
  public blendTrajectory(
    ...
    // Intent Shift / Topic Switch: Queries are divergent.
    // Discard prior trajectory only when cosine similarity falls below driftThreshold.
  ...
  public resolve(
    ...
    // 4. Companion Workflow Boosting (only if companionBoost > 0 is configured)
